...
<body class="bg-gray-200 pt-24">
  ...
  <div class="container mx-auto px-6">
    <h2 class="text-3xl font-bold text-gray-800 mb-6">Approved Requests</h2>
      ...
      <table class="w-full border border-gray-400 shadow-lg rounded-lg">
        <thead class="bg-indigo-600 text-white">
          <tr>
            ...
            <th class="border border-gray-500 p-4 text-left">Approval Date</th>
        ...
        <tbody>
          ...
          $query = "SELECT r.id, u.username, s.item_name, r.quantity, r.created_at 
                    FROM requests r 
                    JOIN users u ON r.staff_id = u.id 
                    JOIN supplies s ON r.item_id = s.id 
                    WHERE r.manager_status = 'approved'";

          $stmt = $conn->prepare($query);

          if (!$stmt) {
              echo "<tr><td colspan='6' class='border border-gray-500 p-4 text-center text-red-500'>Error preparing query.</td></tr>";
          } elseif (!$stmt->execute()) {
              echo "<tr><td colspan='6' class='border border-gray-500 p-4 text-center text-red-500'>Error executing query.</td></tr>";
          } else {
              ...
              if ($result->num_rows > 0) {
                  while ($row = $result->fetch_assoc()) {
                      echo "<tr class='odd:bg-white even:bg-gray-100 hover:bg-gray-200 text-gray-800'>
                              ...
                              <td class='border border-gray-400 p-4'>{$row['created_at']}</td>
                              <td class='border border-gray-400 p-4'>
                                <a href='delete_approved_request.php?id={$row['id']}' class='text-red-600 hover:underline' onclick='return confirm(\"Are you sure you want to delete this request?\")'>Delete</a>
                              </td>
                            </tr>";
                  }
              } else {
                  echo "<tr><td colspan='6' class='border border-gray-400 p-4 text-center text-gray-600'>No approved requests found.</td></tr>";
              }
          }
